  ...
  function validInterval(interval) {
      if (interval <= 10 && interval > 1) {
          ...
          return "Choose a number less than or equal to 10";
      }
  }
  ...
  function displayValidInterval(inputId, outputId) {
      ...
      var outputText = "";
      var random = Math.ceil(Math.random() * input);
      var yourNumber = input2;
      if (validInterval(input) == "okay" && yourNumber == random) {
          outputText = random + "..." + "</br> Looks like your number matched. Do the exercise!";
          document.getElementById(outputId).className = "valid";
      }
      else if (validInterval(input) == "okay" && yourNumber != random) {
          outputText = random + "..." + "</br> Looks like your number did not match. You can skip this task.";
          document.getElementById(outputId).className = "valid";
      }
      else {
          outputText = "Oh c'mon! You must really not want to workout right now. Try again and this time, choose a number less than or equal to 10 :)";
